"""
@file: font_body.py
@description: 检测和修复正文段落的字体名称与字号
              config 示例：{"family": "宋体", "size_pt": 12, "bold": false}
              bold 字段可选，None 表示不检查加粗
@author: Atlas.oi
@date: 2026-04-18
"""
...
# Heading style 名称集合：这些段落不属于正文，不作字体检查
_HEADING_STYLES = {
...
def _read_actual_font_name(run) -> Optional[str]:
    ...
    if rpr is not None and rpr.rFonts is not None:
        ...
        if ascii_name:
            return ascii_name
    # None = 真正继承 doc/style 默认字体（通常为 Calibri）
    return None
...
class FontBodyRule:
    ...
    def detect(doc: Document, value: Any) -> list[Issue]:
        ...
        expected_bold: Optional[bool] = value.get('bold')  # None 表示不检查
        ...
        for p_idx, para in enumerate(doc.paragraphs):
            ...
            for r_idx, run in enumerate(para.runs):
                ...
                issues.append(Issue(
                    ...
                    message='正文字体/字号不符：' + '; '.join(problems),
    ...
    def fix(doc: Document, issue: Issue, value: Any) -> FixResult:
        ...
        # 蓝色标记，提示用户已修改（与 cjk_ascii_space 保持一致的标记风格）
    ...
    def extract(doc: Document) -> dict | None:
        ...
        策略：扫所有非 Heading 段落的所有 run，
        统计 majority 字体名和字号，返回置信度 = 多数票占比。
        """
        ...
        for para in doc.paragraphs:
            ...
            for run in para.runs:
                ...
                if fsize:
                    size_counter[fsize] += 1
        if not family_counter or not size_counter:
            # 文档无正文内容，无法提取
            return None
        family = family_counter.most_common(1)[0][0]
        size = size_counter.most_common(1)[0][0]
        total = sum(family_counter.values())
        conf = family_counter[family] / total if total else 0.0
        return {
            'value': {'family': family, 'size_pt': size},
            'source_xml': f'<w:rFonts w:eastAsia="{family}"/><w:sz w:val="{size * 2}"/>',
