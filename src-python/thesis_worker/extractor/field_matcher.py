...
@description: 字段 id 与段落文本的关联（关键词定位 + 位置兜底）
              ...
              T2.4: 新增 footnote 关键词；
                    使用"脚注格式"/"脚注字体"复合词 + "脚注"基础词；
                    冲突分析：page_footer_number 关键词含"页脚"，"脚注"与"页脚"无子串包含关系
                    （"脚注"≠"页脚"+"注"组合），二者共享"脚"字但文本层面不互相包含，安全；
                    footnote 插在 formula_block 之后、references_title 之前（字典顺序）
@author: Atlas.oi
@date: 2026-04-27
"""
...
# 分组：前置 14 + 正文 11 + 后置 6 + 页面级 6 = 37 个字段（T2.1 新增 table_header，T2.2 拆分 toc_entry，T2.3 新增 formula_block，T2.4 新增 footnote）
FIELD_KEYWORDS: dict[str, list[str]] = {
    ...
    # T2.4: 新增脚注字段关键词。
    # 关键词选择复合词优先，兜底用"脚注"基础词：
    #   - "脚注格式"：规范文本典型表述（"脚注格式：宋体小五号"）
    #   - "脚注字体"：规范文本典型表述（"脚注字体宋体小五"）
    #   - "脚注"：基础词，规范文本中专指页面底部注释，与 page_footer_number 的"页脚"无子串包含
    # 冲突验证：page_footer_number 关键词 ['页脚', '页码'] 不含"脚注"子串；
    #           "脚注"文本不含"页脚"子串；两字段关键词无双向子串包含，顺序无关。
    'footnote': ['脚注格式', '脚注字体', '脚注'],
    # ---- 后置部分（6 个）----
