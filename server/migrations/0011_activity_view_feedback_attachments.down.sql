-- @file 0011_activity_view_feedback_attachments.down.sql
-- @description 回滚 0011：恢复 0010 view（feedback 仅有 attachmentCount，无 attachments 数组）
-- @author Atlas.oi
-- @date 2026-05-03
...
FROM feedbacks f
LEFT JOIN (SELECT feedback_id, COUNT(*) AS cnt FROM feedback_attachments GROUP BY feedback_id) att
    ON att.feedback_id = f.id
UNION ALL
