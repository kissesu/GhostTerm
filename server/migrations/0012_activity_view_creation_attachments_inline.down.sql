-- @file 0012_activity_view_creation_attachments_inline.down.sql
-- @description 回滚 0012：恢复 0011 视图（无 inline 附件 + 不过滤 E0/wechat_chat）
-- @author Atlas.oi
-- @date 2026-05-03
...
FROM feedbacks f
LEFT JOIN (SELECT fa.feedback_id,
    jsonb_agg(jsonb_build_object('id', files.id, 'filename', files.filename) ORDER BY fa.id) AS list,
    COUNT(*) AS cnt FROM feedback_attachments fa JOIN files ON files.id = fa.file_id
    GROUP BY fa.feedback_id) att ON att.feedback_id = f.id
UNION ALL
...
FROM (SELECT s.*, EXTRACT(EPOCH FROM (s.triggered_at - LAG(s.triggered_at) OVER (
    PARTITION BY s.project_id ORDER BY s.triggered_at))) * 1000 AS dwell_ms
    FROM status_change_logs s) sc
